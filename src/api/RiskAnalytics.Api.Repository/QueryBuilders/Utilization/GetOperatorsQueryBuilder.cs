...
public class GetOperatorsQueryBuilder : IGetOperatorsQueryBuilder
{
    public string BuildQuery(MonthlyUtilizationGroup? groupBy)
    {
        ...
        queryBuilder.AppendLine(@$"
            ...
            SELECT DISTINCT
                aircraft_all_history.operator_organization_id id,
                aircraft_all_history.operator name
            ...
            JOIN ""organizations_latest"" as organizations ON aircraft_all_history.operator_organization_id = organizations.organization_id
            WHERE
                ...
                (
                    ...
                    (
                        ...
                        AND organizations.organization_sub_type_id IN (
                            {(int)OrganizationSubType.Mainline},
                            {(int)OrganizationSubType.Leisure},
                            {(int)OrganizationSubType.Regional},
                            {(int)OrganizationSubType.LowCost},
                            {(int)OrganizationSubType.Cargo},
                            {(int)OrganizationSubType.Other},
                            {(int)OrganizationSubType.AirTaxiCharter},
                            {(int)OrganizationSubType.AirlineACMI}
                        )
                    )
                )");
        ...
        queryBuilder.AppendLine($"AND (:lessorId IS NULL OR aircraft_all_history.manager_organization_id = :lessorId)");

        queryBuilder.AppendLine("ORDER BY aircraft_all_history.operator");
